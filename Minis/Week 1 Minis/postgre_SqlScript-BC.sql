--to solve the problem of recreating tables use this comment 
drop table if exists transaction, product_type, product, officer, individual, employee, department, department, customer, business, branch, account;
drop type if exists enum_status, enum_txn_type_cd, Enum_IB;

--start creating tble/data population 
...
create type Enum_IB as enum('I','B');

create table customer
 (cust_id SERIAL not null,
  fed_id varchar(12) not null,
  cust_type_cd Enum_IB not null,
  address varchar(30),
  city varchar(20),
  state varchar(20),
...
create type enum_status as enum('ACTIVE','CLOSED','FROZEN');
...
create type enum_txn_type_cd as enum('DBT','CDT');

create table transaction
 (txn_id SERIAL  not null,
  txn_date timestamp not null,
  account_id integer  not null,
  txn_type_cd enum_txn_type_cd,
...
select a.prod_cd, c.cust_id, a.open_date, a.last_date, 'ACTIVE', e.branch_id, e.emp_id, a.avail, a.pend from customer c cross join 
 ...
 (select 'CHK' prod_cd, to_date('2000-01-15','yyyy-mm-dd') open_date, to_date('2005-01-04','yyyy-mm-dd') last_date,
  ...
  select 'SAV' prod_cd, cast('2000-01-15' as date) open_date, cast('2004-12-19' as date) last_date,
    500.00 avail, 500.00 pend union all
  select 'CD' prod_cd, cast('2004-06-30' as date) open_date, cast('2004-06-30' as date) last_date,
...
select a.prod_cd, c.cust_id, cast(a.open_date as date) as open_date, cast(a.last_date as date) as last_date, 'ACTIVE',
...
select cast(a.open_date as date) as open_date, a.account_id, 'CDT', 100, cast(a.open_date as date) as open_date
from account a
